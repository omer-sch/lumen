...
//
// The world-atlas TopoJSON keys countries by the numeric code on
// `feature.id` (e.g. "840" for the United States), while our BQ data
// rows key by the alpha-2 code from `src/lib/iso-country-codes.ts`
// (e.g. "US"). This map bridges the two so the choropleth can
// fill-by-value without a name-string fuzzy match.
...
/** Convert ISO 3166-1 numeric (zero-padded or not) to alpha-2. Returns
 *  null for codes outside the spec (Antarctica, disputed regions, etc.)
 *  so callers can gracefully treat those as "no data". */
export function alpha2FromNumeric(numeric: string | number | null | undefined): string | null {
