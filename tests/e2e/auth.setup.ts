...
//   E2E_CLERK_USER_USERNAME        — pre-provisioned test user identifier
//                                    (email or username, depending on Clerk
//                                    instance config)
  ...
  if (!username || !password) {
    throw new Error(
      "E2E_CLERK_USER_USERNAME and E2E_CLERK_USER_PASSWORD must be set " +
        "to run the authenticated Playwright project. See the comment at " +
        "the top of tests/e2e/auth.setup.ts.",
    );
  }
